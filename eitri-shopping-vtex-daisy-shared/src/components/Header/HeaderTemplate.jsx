  ...
  return (
    ...
    >
      {headerType === HEADER_TYPE.LOGO_SEARCH_AND_CART && (
        <>
          <HeaderLogo src={logo || urlLogo} height={40} />
          <View className="flex flex-row justify-end">
            <HeaderSearchIcon navigateToSearch={navigateToSearch} iconColor={contentColor} />
            <HeaderCart
              backgroundColor={backgroundColor}
      ...
      {headerType === HEADER_TYPE.RETURN_SEARCH_AND_CART && (
        <>
          <HeaderReturn backgroundColor={backgroundColor} iconColor={contentColor} />
          <HeaderSearch labelSearch={labelSearch} onClick={navigateToSearch} />
          <HeaderCart
            width={'20%'}
            backgroundColor={backgroundColor}
            quantityOfItems={quantityOfItems}
      ...
      {headerType === HEADER_TYPE.TEXT_AND_SEARCH_ICON && (
        <>
          <HeaderText text={contentText} contentColor={contentColor} />
          <HeaderSearchIcon iconColor={contentColor} navigateToSearch={navigateToSearch} />
        </>
      )}
      ...
      {headerType === HEADER_TYPE.RETURN_TEXT_FILTER_AND_SEARCH_ICON && (
        <>
          {viewBackButton && <HeaderReturn backgroundColor={backgroundColor} iconColor={contentColor} />}
          <HeaderText contentColor={contentColor} text={contentText} />
          <View className="flex">
            {filterFacets && (
              <HeaderFilter
                iconColor={contentColor}
      ...
      {headerType === HEADER_TYPE.RETURN_SHARE_AND_CART && (
        <>
          <HeaderReturn iconColor={contentColor} backgroundColor={backgroundColor} />
          <View className="flex">
            <HeaderShare iconColor={contentColor} handleShare={handleShare} />
            <HeaderCart
              backgroundColor={backgroundColor}
              quantityOfItems={quantityOfItems}
              onClick={navigateToCart}

    ...
    <View className="flex items-center">
      <View className="flex flex-row items-center">
        {[1, 2, 3, 4, 5].map((star, index) => {
          return (
            <View key={index} onClick={() => handleRating(star)} className="flex flex-row items-center">
      ...
      {ratingsCount && <Text className="block w-full text-secondary-content">{`(${ratingsCount})`}</Text>}
    </View>
  )
}
